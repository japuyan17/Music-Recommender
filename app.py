...
def generate_data(n_users=N_USERS, n_artists=N_ARTISTS, interactions_per_user=30, seed=42):
    ...
    for user in range(n_users):
        ...
        for artist in pool:
            rows.append(user); cols.append(int(artist))
...
# build user-item matrix for history lookup
...
def index():
    return render_template('index.html',
        n_users=N_USERS, n_artists=N_ARTISTS,
        model_meta=MODEL_META, winner=WINNER)
...
def recommend():
    ...
    return jsonify({'user_id': user_id, 'model': model,
